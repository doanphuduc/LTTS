...
long long Hash(int k, int l, int r) {
    return (H[k][r] - H[k][l - 1] * pw[r - l + 1] + 1LL * BASE * BASE) % BASE;
}
...
int main() {
    cin >> n;
    string T; cin >> T;
    n = n * 2 + 1; // remember doubling memory when doubling string
    for (int i = 1; i <= n; ++i)
        S[i] = (i & 1) ? '#' : T[i / 2 - 1]; // -1 do string T 0-indexed
    pw[0] = 1;
    for (int i = 1; i <= n; ++i) {
        pw[i] = pw[i - 1] * B % BASE;
        H[0][i] = (H[0][i - 1] * B + S[i]) % BASE;
        H[1][i] = (H[1][i - 1] * B + S[n - i + 1]) % BASE;
    }
    int ans = 0;
    for (int i = 1; i <= n; ++i) {
